...
const CLIENT_COOKIE = 'ielts_vocab_client'
...
async function rateLimitResponse(resetAt: number) {
  await recordSiteEvent('vocab_audio_rate_limited').catch(() => {})
...
export async function GET(request: NextRequest) {
  ...
  if (!key || !key.startsWith('ielts-vocab/') || !key.endsWith('.mp3') || key.includes('..')) {
    return NextResponse.json(
      { success: false, error: '无效的音频参数' },
  ...
  const clientLimit = rateLimit(`vocab-audio:client:${clientHash}`, {
    interval: 60_000,
    maxRequests: 60,
  })
  if (!clientLimit.allowed) return rateLimitResponse(clientLimit.resetAt)

  const ipLimit = rateLimit(`vocab-audio:ip:${ipHash}`, {
    interval: 60_000,
    maxRequests: 200,
  })
  ...
  try {
    ...
    await recordSiteEvent('vocab_audio_signed').catch(() => {})

    const response = NextResponse.json(
      {
        success: true,
        data: { url, expiresIn: SIGNED_URL_EXPIRES_SECONDS },
      },
      {
        headers: {
    ...
    console.error('Failed to create COS signed URL for vocab audio:', error)
    return NextResponse.json(
